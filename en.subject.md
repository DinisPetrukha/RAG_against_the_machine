...
- [V. Common Instructions](#v-common-instructions)
  - [V.1 General Rules](#v1-general-rules)
  - [V.2 Makefile](#v2-makefile)
  - [V.3 Additional Guidelines](#v3-additional-guidelines)
  - [V.4 Additional Requirements](#v4-additional-requirements)
- [VI. Mandatory part](#vi-mandatory-part)
  - [VI.1 Indexing the codebase](#vi1-indexing-the-codebase)
  - [VI.2 Retrieval](#vi2-retrieval)
  - [VI.3 Answer generation](#vi3-answer-generation)
  - [VI.4 Data Models](#vi4-data-models)
  - [VI.5 Output](#vi5-output)
  - [VI.6 Command-Line Interface](#vi6-command-line-interface)
  - [VI.7 End-to-end walkthrough](#vi7-end-to-end-walkthrough)
    - [VI.7.1 Expected project layout for evaluation](#vi71-expected-project-layout-for-evaluation)
    - [VI.7.2 Running the full pipeline](#vi72-running-the-full-pipeline)
- [VII. Evaluation](#vii-evaluation)
  - [VII.1 Evaluation metrics](#vii1-evaluation-metrics)
    - [VII.1.1 Recall@k Calculation](#vii11-recallk-calculation)
    - [VII.1.2 Performances](#vii12-performances)
- [VIII. Readme Requirements](#viii-readme-requirements)
- [IX. Bonus Part](#ix-bonus-part)
- [X. Submission and peer-evaluation](#x-submission-and-peer-evaluation)
  - [X.1 Recode instructions](#x1-recode-instructions)
...
![RAG pipeline: Indexing (Corpus → Chunking) → Index (BM25/TF-IDF) → Retrieval and Generation (Retrieve top-k sources → Augment → Generate with Qwen3-0.6B → Answer JSON), with a scoring branch from Retrieve to an Evaluate recall@k step.](assets/rag-pipeline-diagram.png)
...
**Example — single-query search** returns ranked source locations, each with its file path and character span:

![Terminal example: `uv run python -m src search "How to configure the OpenAI server?" --k 5` printing five ranked file_path [start:end] results.](assets/example-search.png)
...
Below is another example of the moulinette's output on a different run, illustrating the same format with different figures:

![Terminal example: ./moulinette evaluate_student_search_results ... --k 10, printing "Student data is valid: True", question counts, and Recall@1/3/5/10 with a note that the docs threshold is 80%.](assets/example-evaluate.png)

And an example of the `index` command's own progress output:

![Terminal example: uv run python -m src index --max_chunk_size 2000, showing tqdm progress bars for Chunking and Tokenizing, ending with "Ingestion complete! Indexed 13466 chunks under data/processed/".](assets/example-indexing.png)
